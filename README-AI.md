...
**IMPORTANT**: This project uses Arduino IDE for compilation and upload. AI assistants must follow this workflow:

1. **Code Changes**: Make all code modifications in the current environment
2. **User Notification**: ALWAYS prompt the user to compile and upload in Arduino IDE
...
- "Please compile and upload the code in Arduino IDE"
...
1. **Main Controller** (`eyes-v0.1.ino`)
...
5. Create new behavior file following the pattern of `BehaviorPumpkin.ino` or `BehaviorMouthInvasion.ino`
...
- **Primary IDE**: Arduino IDE for compilation and upload
- **Development**: Code editing typically done in separate environment (VS Code, etc.)
- **Workflow**: Edit code → Arduino IDE → Compile → Upload → Test
...
2. AI prompts user to compile and upload in Arduino IDE
